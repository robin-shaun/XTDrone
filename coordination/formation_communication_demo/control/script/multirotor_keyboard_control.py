...
from control.msg import UAVDataMsg, AllUAVData
...
if __name__=="__main__":
    ...
    elif multirotor_num == 20:
        formation_configs = ['waiting','FORM_1','FORM_2','FORM_3']    
    
    buffer_uav_state_sub = [None]*multirotor_num
    ...
    for i in range(multirotor_num):
        all_uav_state.data.append(UAVDataMsg());
        buffer_uav_state_sub[i] = rospy.Subscriber("/UAV"+str(i)+"/formation/uav_data",UAVDataMsg, uavstate_callback)
    
    uav_state_pub = rospy.Publisher("xtdrone_gcs/all_uav_data", AllUAVData, queue_size=10)
