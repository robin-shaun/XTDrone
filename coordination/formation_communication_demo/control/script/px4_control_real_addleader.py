...
* To control a UAV in real environment
...
$ mpiexec -n 3 python2.7 px4_control_real_addleader.py            # 3 UAVs in simulation
...
class Px4Controller:
    ...
    def working(self):
        ...
        while rospy.is_shutdown() is False:
            ...
            if self.gcs_cmd == "AUTO.TAKEOFF":
                self.form_flag = 0
                self.target_pose = self.construct_target(0, 0, self.takeoff_height, self.current_heading)
                ...
                # if self.uav_id == 0:
                #     if self.uav_data.pose.position.y < 5.0:
                ...
                self.formation_control()
                # if self.uav_id == 0:
                #     if self.uav_data.pose.position.y > 0.0:
                #         self.target_vel.twist.linear.x = 0
                #         self.target_vel.twist.linear.y = -0.
                #         self.target_vel.twist.linear.z = 0
                #     else:
                ...
                self.form_flag = 0
                self.state = self.flight_mode_set(mode=self.gcs_cmd)
                if (self.state is "LAND") and (self.local_pose.pose.position.z < 0.15):
                    if self.disarm():
                        self.state = "DISARMED"
            else:
                self.form_flag = 0
                self.state = self.flight_mode_set(mode='AUTO.LOITER')
    ...
    def takeoff_detection(self):
        if self.local_pose.pose.position.z > 0.1 and self.arm_state:
            return True
        else:
            return False
