...
void MainWindow::ReadSettings() {
    ...
    connect(ui.button_stop,SIGNAL(clicked(bool)),this,SLOT(slot_btn_stop_click(bool)));
...
void MainWindow::Seriesint(int ch)
{
    ...
    seriesList[ch]->setPen(QPen(color,1,Qt::SolidLine));
...
//    connect(f,SIGNAL(turn_to_main(bool)),this,SLOT(slot_init(bool)));
    ...
    if (once_flag==1)
    {
        ...
        for (int i = 0; i < multi_type_num; i++)
        {
            ...
            //if ((multirotor_num[i] == 3)||(multirotor_num[i] == 6) || (multirotor_num[i] == 9)||(multirotor_num[i] == 10)||(multirotor_num[i] == 13) || (multirotor_num[i] == 2))
            if (multirotor_num[i]<15)
            {
//                if (formation_flag)
//                {
...
//                  ui.button_add->setEnabled(true);
            ...
            {
                if ( ! qnode.init2(ui.line_edit_master->text().toStdString(),ui.line_edit_host->text().toStdString(), multirotor_select, multirotor_num, multi_type,control_type.toStdString()))
                {
                    showNoMasterMessage();
                    ui.treeWidget->setEnabled(false);
                }
...
{
    //if (multirotor_formation_num == 3 || multirotor_formation_num == 6 || multirotor_formation_num == 10 || multirotor_formation_num == 13 || multirotor_formation_num == 2)
    if ((multirotor_formation_num!=9) && (multirotor_formation_num<15))
    {
        if (formation_mid == "formation1")
        {
            formation2 = "T";
        }
        else if (formation_mid == "formation2")
        {
            formation2 = "diamond";
        }
        else if (formation_mid == "formation3")
        {
            formation2 = "triangle";
        }
        else
...
//    qDebug()<<"i got it";
    ...
    if (plot_plot_count > 50)
        seriesList[plot_count]->remove(0);
