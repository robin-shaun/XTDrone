...
bool QNode::init2(const std::string &master_url, const std::string &host_url, const LISTINT multi_select, const int *multi_num, const LISTSTR multi_type, std::string q_control_type) {
//	ros::init("xtdrone_qt");
    int leng_select = multi_select.size();
    int counnnt = 0;
    int counnnnnt = 0;
...
//        qDebug()<<"multi_control:"<<multirotor_get_control;
    ...
    { 
        ...
        {
            cmd_pub.publish(xtdrone_cmd);
            qDebug()<<"cmdxtd:"<<QString::fromStdString(xtdrone_cmd.data);
        }
        if (xtdrone_cmd.data =="T")
        {
            xtdrone_cmd.data = "FORM_1";
            cmd_pub.publish(xtdrone_cmd);
            qDebug()<<"cmdxtd:"<<QString::fromStdString(xtdrone_cmd.data);
        }
        if (xtdrone_cmd.data =="diamond")
        {
            xtdrone_cmd.data = "FORM_2";
            cmd_pub.publish(xtdrone_cmd);
            qDebug()<<"cmdxtd:"<<QString::fromStdString(xtdrone_cmd.data);
        }
        if (xtdrone_cmd.data == "triangle")
        {
            xtdrone_cmd.data = "FORM_3";
            cmd_pub.publish(xtdrone_cmd);
            qDebug()<<"cmdxtd:"<<QString::fromStdString(xtdrone_cmd.data);
        }
    ...
    last_cmd.data = q_cmd;
//    if (ctrl_leader != q_ctrl_leader)
//    {
//        if (ctrl_leader)
//            xtdrone_cmd.data = "control the leader";
//        else
//            xtdrone_cmd.data = "control all";
//        cmd_pub.publish(xtdrone_cmd);
//    }
...
//    geometry_msgs::Twist twist[multirotor_num];
    qDebug()<<"start!!!";
    while ( ros::ok() ) {
        //rviz draw path and publish alluav_state
        if (count%2==0)
        {
            current_time = ros::Time::now();
