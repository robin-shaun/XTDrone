...
def create_point():
    ...
    size_box_judge=[[16,7],[1,1],[2,1],[2,1],[20, 30],[30, 17],[16,12],[16,12],[16,16],[11, 9],[11, 9],[11, 9],[17, 6]]
    in_obstacle = True
    for m in range(9):
        while in_obstacle:
            if m==0:
            ...
            for i in range(box_len):
                if (abs(x-center_box_judge[i][0]) > (size_box_judge[i][0]+size_box_judge[box_len][0])/2+2) or (abs(y-center_box_judge[i][1]) > (size_box_judge[i][1]+size_box_judge[box_len][1])/2+2):
                    in_obstacle=False
                else:
                    in_obstacle=True
                    break
...
def create_human_point(black_box):
    ...
    while in_obstacle:
        ...
        for i in range(int(len(black_box))):  
            if (a > (black_box[i][0][0]-3)) and (a < (black_box[i][0][1]+3)) or (b > (black_box[i][1][0]-3)) and (b < (black_box[i][1][1]+3)):
                in_obstacle = True
                break
            else:
                in_obstacle=False
...
## outdoor.world
content=open(world_file_path+"robocup.world",'r')
with open(world_file_path+"outdoor_house.world",'w') as f:
    count = 5
    change_flag=False
    ...
    for num,line in enumerate(lines):
        for i in range(len(name_list)):
            ...
            if change_flag and "<pose frame=''>" in line and "<link name='link'>" not in lines[num-1]:
                pose_ori=line.split()
                line = "<pose frame=''>"+str(center_list[i][0])+' '+str(center_list[i][1])+' '+str(pose_ori[-4])+' '+str(pose_ori[-3])+' '+str(pose_ori[-2])+' '+str(pose_ori[-1])+'\n'
                change_flag = False
            if  "<model name='"+name_list[i]+"'>\n" in line:
                if "<static>1</static>\n" in lines[num+1]:
                    ...
                    line_1 ="<pose frame=''>"+str(center_list[i][0])+' '+str(center_list[i][1])+' '+str(pose_ori[-4])+' '+str(pose_ori[-3])+' '+str(pose_ori[-2])+' '+str(pose_ori[-1])+'\n'
        count=count+1
        f.write(line)
f.close()
## random_rover
rover_center_list=create_rover_point(rover_num)
rover_content = open(world_file_path+"rover_static.world",'r')
with open(world_file_path+"rover.world",'w') as f:
        ...
        for num, line in enumerate(lines):
            if count ==1 and num+1 < len(lines):
                if "</model>" in lines[num+1]:
                    pose_ori = line.split()
                    line = " <pose frame=''>"+str(rover_center_list[i][0])+' '+str(rover_center_list[i][1])+' '+str(pose_ori[-4])+' '+str(pose_ori[-3])+' '+str(pose_ori[-2])+' '+str(pose_ori[-1])+'\n' 
                    if count == 2 and num>0:
                        if "<model name='rover_static" in line:
                            ...
                            pose_ori = line.split()
                            line = " <pose frame=''>" + str(rover_center_list[i][0]-1.2293) + ' ' + str(rover_center_list[i][1]-0.6585) + ' ' + str(pose_ori[-4]) + ' ' + str(pose_ori[-3]) + ' ' + str(pose_ori[-2]) + ' ' + str(pose_ori[-1]) + '\n'
                        elif "<link name='front_left_wheel_steering_block'>" in lines[num-1]:
                            pose_ori=line.split()
                            line = " <pose frame=''>" + str(rover_center_list[i][0] - 1.2189) + ' ' + str(rover_center_list[i][1] - 0.4431) + ' ' + str(pose_ori[-4]) + ' ' + str(pose_ori[-3]) + ' ' + str(pose_ori[-2]) + ' ' + str(pose_ori[-1]) + '\n'
                        elif "<link name='front_right_wheel'>" in lines[num-1]:
                            pose_ori = line.split()
                            line = " <pose frame=''>" + str(rover_center_list[i][0] - 1.1614) + ' ' + str(rover_center_list[i][1] + 0.7712) + ' ' + str(pose_ori[-4]) + ' ' + str(pose_ori[-3]) + ' ' + str(pose_ori[-2]) + ' ' + str(pose_ori[-1]) + '\n'
...
#content=open("/home/robocup/PX4_Firmware/Tools/sitl_gazebo/worlds/outdoor_house.world",'r')
content=open(world_file_path+"outdoor_house.world",'r')
rover_content = open(world_file_path+"rover.world",'r')
with open(output_path+"outdoor.world",'w') as f:
    lines=content.readlines()
...
ARRS = []
#f=open('/home/robocup/actors_plugings_for_test/src/obstacle.txt','w+')
f=open(actor_avoid_file_path+'obstacle.txt','w+')
