...
if __name__ == "__main__":
    ...
    test_pub_red2 = rospy.Publisher("/actor_red2_info",ActorInfo,queue_size=1)
    test_pub_green = rospy.Publisher("/actor_green_info",ActorInfo,queue_size=1)
    test_pub_red1 = rospy.Publisher("/actor_red1_info",ActorInfo,queue_size=1)
    test_pub_brown = rospy.Publisher("/actor_brown_info",ActorInfo,queue_size=1)
    test_pub_blue = rospy.Publisher("/actor_blue_info",ActorInfo,queue_size=1)
    test_pub_white = rospy.Publisher("/actor_white_info",ActorInfo,queue_size=1)
    get_model_state = rospy.ServiceProxy("/gazebo/get_model_state",GetModelState)
    left_actors_sub = rospy.Subscriber("/left_actors",String,left_actors_callback,queue_size=1)
    left_actors = range(6)
    rate = rospy.Rate(2)
    time.sleep(1)
