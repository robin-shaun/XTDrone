import rospy
from geometry_msgs.msg import PoseStamped
from nav_msgs.msg import Odometry
import math
from pyquaternion import Quaternion

local_pose = PoseStamped()
local_pose.header.frame_id = 'map'

def vins_callback(data):
    #rospy.loginfo(str(data.pose.position.x)+','+str(data.pose.position.y)+','+str(data.pose.position.z))
    local_pose.pose.position.x = data.pose.pose.position.z
    local_pose.pose.position.y = -data.pose.pose.position.y
    local_pose.pose.position.z = data.pose.pose.position.x
    local_pose.pose.orientation.x = data.pose.pose.orientation.x
    local_pose.pose.orientation.y = data.pose.pose.orientation.y
    local_pose.pose.orientation.z = data.pose.pose.orientation.z
    local_pose.pose.orientation.w = data.pose.pose.orientation.w

rospy.init_node('vins_transfer')
rospy.Subscriber("/vins_estimator/camera_pose", Odometry, vins_callback)
position_pub = rospy.Publisher("/mavros/vision_pose/pose", PoseStamped, queue_size=2)
rate = rospy.Rate(60) 

while True:
    local_pose.header.stamp = rospy.Time.now()
    position_pub.publish(local_pose) 
    rate.sleep()
  
